 ...
 */
class Bookingbus extends MX_Controller
{
   ...
   public function index()
    {
        ...
        $crud->callback_field('status', array($this, 'status_page'));
        ...
        if ($table->uploads != '') {
            ...
            foreach ($fields_upload as $field_upload) {
                $crud->set_field_upload($field_upload, 'uploads/formbooking');
            }
        }
        ...
        "path_to_directory"       =>'uploads/formbooking',
     ...
     * @return HTML
     **/
    public function status_page($value = '', $primary_key = null)
    {
        $checked['front'] = '';
        $checked['back'] = '';
        if ($value != '') {
        ...
        $front = '<label><input type="radio" name="status" value="Request" '.$checked['front'].' class="check">Request</label>';
        $back = '<label><input type="radio" name="status" value="Approve" '.$checked['back'].' class="check">Approve</label>';
